...
{    
  /* Start Exp1 ANN/CTRNN*/
  double fit = ( m_fComputedFitness / (double) m_unNumberOfSteps );
  /* End Exp1 */
  
  /* Start Exp2 ANN/CTRNN*/
  //double fit = ( m_fComputedFitness / (double) m_unNumberOfSteps ) * (1 - ((double) (fmin(m_unCollisionsNumber,10.0)/10.0)));
  /* End Exp2 */

  /* Start Exp3-4 ANN/CTRNN*/
  //double fit = ( m_fComputedFitness / (double) m_unNumberOfSteps ) * (1 - ((double) (fmin(m_unCollisionsNumber,10.0)/10.0))) * ( (double) (fmin(m_unGreyCounter, 20.0)/ 20.0 ));
  /* End Exp3-4 */
  ...
  /* Start Exp5 CTRNN with += in several tests in main.cpp*/
  //double fit = ( m_fComputedFitness / (double) m_unNumberOfSteps ) * (1 - ((double) (fmin(m_unCollisionsNumber,10.0)/10.0))) * ( (double) (fmin(m_unGreyCounter, 20.0)/ 20.0 ));
  /* End Exp5 CTRNN */
  ...
  //if (m_unGreyFlag == 1 )
    //fit /= 10.0;
  ////printf("----FIT %2f ---- GreyCounter: %d --- Colli: %d\n", fit, m_unGreyCounter, m_unCollisionsNumber);
  /* End Exp6 */
...
void CGarbageFitnessFunction::SimulationStep(unsigned int n_simulation_step, double f_time, double f_step_interval)
{
 ...
 /* Maximize movement as in avoidfitnessfunction */
  ...
  //fitness *= maxProxSensorEval * (leftSpeed * rightSpeed); 
	/* END Garbage Exp 5-6 */
  
  /* START Garbage Exp CTRNN 6A */
  //double fitness = 1; //0.5 * (maxSpeedEval * sameDirectionEval);
  //fitness *= (( blueLightS0 + blueLightS7)/2);
  /* END Garbage Exp CTRNN 6A */

  /* START Garbage Exp CTRNN 6B */
  //double fitness = 1; //0.5 * (maxSpeedEval * sameDirectionEval);
  ...
  //{
    //fitness *= (( blueLightS0 + blueLightS7)/2);
  //}
	/* END Garbage Exp CTRNN 6B */

  /* START Garbage Exp CTRNN 6C */
  //double fitness = 1; //0.5 * (maxSpeedEval * sameDirectionEval);
  ...
  //{
    ...
    //}
  //}
	/* END Garbage Exp CTRNN 6C */
  
  /* START Garbage Exp CTRNN 7-8 */
  //double fitness = 1; //0.5 * (maxSpeedEval * sameDirectionEval);
